...
var quantityBlue = 10; //ile ma byæ u¿ytych niebieskich fasolek 
var limitPA = 50000 // iloœæ PA przy której u¿ywana jest fasolka 
...
const $html = "<div class='gh_button gh_exp'>Resp <b class='gh_status red'>Off</b></div><div class='gh_button gh_xost'>xOst <b class='gh_status red'>Off</b></div><div class='gh_button gh_x20'>x20 <b class='gh_status red'>Off</b></div><div class='gh_button gh_senzu'>Senzu <b class='gh_status red'>GREEN</b></div>";
...
$('#gh_game_helper .gh_senzu').click(() => {
	if (useBlueBeans) {
		...
		$('#gh_game_helper .gh_senzu')
		$(".gh_senzu .gh_status").removeClass("red").addClass("red").html("GREEN");
		useRedBeans = false;
        useBlueBeans = true;
	}	
});
...
function use_blue(x){
    var blue = GAME.quick_opts.senzus.find(senzu => senzu.item_id === 1242);
	setTimeout(() => {
		GAME.emitOrder({a:12,type:14,iid:blue.id,page:GAME.ekw_page,am:x});
		stop = false;
	}, 1000);

	setTimeout(() => { usedSenzu = false; }, 90000);
}
