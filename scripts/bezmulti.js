...
function fight (mob_num = 0) {
	...
	// check if mob exists on field and has no multi fight yet
        else if (GAME.field_mobs[mob_num].ranks[0]) fightNormal(mob_num) // kill normal if exists
	else if (GAME.field_mobs[mob_num].ranks[1]) fightChampion(mob_num) // kill champion if exists
        else if (GAME.field_mobs[mob_num].ranks[2]) fightElita(mob_num) // kill elita if exists
...
function fightNormal (mob_num = 0) {
	GAME.emitOrder({a: 7, mob_num: mob_num, rank: 0, quick: 1});
}

function fightChampion (mob_num = 0) {
	GAME.emitOrder({a: 7, mob_num: mob_num, rank: 1, quick: 1});
}

function fightElita (mob_num = 0) {
