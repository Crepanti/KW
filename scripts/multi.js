...
charsLabel.textContent = 'POSTACIE:';
charsLabel.style.display = 'block';
charsLabel.style.marginBottom = '5px';

const charsSelect = document.createElement('select');
const charsOptions = nicknames;
for (let i = 0; i < charsOptions.length; i++) {
  const option = document.createElement('option');
  option.value = i+1;
  option.text = charsOptions[i];
  charsSelect.appendChild(option);
}
charsSelect.style.display = 'block';
charsSelect.style.marginBottom = '10px';
charsSelect.style.width = '100%';
charsSelect.style.padding = '5px';
charsSelect.style.borderRadius = '5px';
charsSelect.style.background = '#444';
charsSelect.style.color = '#fff';
charsSelect.style.border = 'none';
*/
...
/*
panelek.appendChild(charsLabel);
panelek.appendChild(charsSelect);
*/
